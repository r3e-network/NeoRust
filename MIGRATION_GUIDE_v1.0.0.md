# NeoRust SDK v1.0.0 Migration Guide

## Overview

This guide provides step-by-step instructions for migrating existing projects from pre-1.0 releases to v1.0.0, which includes significant enhancements in gas estimation, rate limiting, testing infrastructure, and production readiness features.
...
Version 1.0.0 maintains backward compatibility with pre-1.0 releases. The only changes required are:
...
2. **Import Path Updates**: Update version references in documentation
   - Change `neo3 = "1.0.0"` to `neo3 = "1.0.0"` in Cargo.toml
