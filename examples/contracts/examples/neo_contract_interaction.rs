...
async fn connect_to_testnet() -> Result<neo3::providers::RpcClient<neo3::providers::HttpProvider>, Box<dyn std::error::Error>> {
	let endpoints = vec![
		...
		match client.get_contract_state(&script_hash).await {
			Ok(state) => {
				...
				if let Some(manifest) = &state.manifest {
					println!("      • Methods: {}", manifest.abi.as_ref()
						.map(|abi| abi.methods.len()).unwrap_or(0));
					println!("      • Events: {}", manifest.abi.as_ref()
						.map(|abi| abi.events.len()).unwrap_or(0));
				}
			},
...
async fn interact_with_nep17_token(
		...
		Ok(result) => {
			if let Some(stack) = result.stack {
				if let Some(item) = stack.first() {
					println!("      • Symbol: {}", 
						String::from_utf8(hex::decode(item.value.as_ref()
							.and_then(|v| v.as_str())
							.unwrap_or("474153"))?)
	...
	// Get decimals
	match client.invoke_function(&gas_hash, "decimals", &[]).await {
		Ok(result) => {
			if let Some(stack) = result.stack {
				if let Some(item) = stack.first() {
					println!("      • Decimals: {}", 
	...
	match client.invoke_function(&gas_hash, "balanceOf", &[
		neo3::neo_types::ContractParameter::hash160(&address_hash),
	]).await {
		Ok(result) => {
			if let Some(stack) = result.stack {
				if let Some(item) = stack.first() {
					let balance = item.value.as_ref()
						.and_then(|v| v.as_str())
						.and_then(|s| s.parse::<i64>().ok())
						.unwrap_or(0);
					println!("      • Balance of {}: {} GAS", 
						example_address, balance as f64 / 100_000_000.0);
				}
			}
		},
...
async fn demonstrate_contract_invocation(
	...
	let nns_hash = neo3::neo_types::ScriptHash::from_str("50ac1c37690cc2cfc594472833cf57505d5f46de")?;
	let domain = "neo.neo";
	
	match client.invoke_function(&nns_hash, "resolve", &[
	...
	script_builder.contract_call(
		...
		neo3::neo_builder::CallFlags::All,
	)?;
...
async fn demonstrate_transaction_building(
	...
	script_builder.contract_call(
		...
		"transfer",
		&[
			neo3::neo_types::ContractParameter::hash160(&neo3::neo_types::ScriptHash::from_address(sender)?),
			neo3::neo_types::ContractParameter::hash160(&neo3::neo_types::ScriptHash::from_address(recipient)?),
			neo3::neo_types::ContractParameter::integer(amount),
			neo3::neo_types::ContractParameter::any(serde_json::Value::Null),
		],
		neo3::neo_builder::CallFlags::All,
	)?;
	...
	tx_builder
		...
		.add_signer(neo3::neo_builder::Signer {
			account: neo3::neo_types::ScriptHash::from_address(sender)?,
...
async fn demonstrate_advanced_patterns(
	...
	script_builder.contract_call(
		gas_hash,
		"balanceOf",
		&[neo3::neo_types::ContractParameter::hash160(&neo3::neo_types::ScriptHash::from_address("NPvKVTGZapmFWABLsyvfreuqn73jCjJtN1")?)],
		neo3::neo_builder::CallFlags::ReadOnly,
	)?;
