...
export function MintNFTModal({
  ...
  const validateForm = () => {
    ...
    if (
      ...
      !isValidHexColor(formData.background_color)
    ) {
      newErrors.background_color =
        'Please enter a valid hex color (e.g., #FF0000)';
    }

    // Validate attributes
    const invalidAttributes = formData.attributes.some(
      attr =>
    ...
    if (invalidAttributes) {
      newErrors.attributes =
        'All attributes must have both trait type and value, or leave both empty';
    }
                  ...
                  {formData.attributes.length === 0 ? (
                    <div className='text-center py-8 text-gray-500 border-2 border-dashed border-gray-200 rounded-lg'>
                      ...
                      <p className='text-xs mt-1'>
                        Attributes help describe unique traits of your NFT
                      </p>
                    </div>
                  ) : (
              ...
              <div className='flex items-start'>
                ...
                <div className='ml-3'>
                  ...
                  <div className='mt-2 text-sm text-blue-700'>
                    <p>
                      • Listing price: {price || '0'} {currency}
                    </p>
                    <p>• Duration: {duration} days</p>
                    <p>• Platform fee: 2.5%</p>
                    <p>
                      • You will receive:{' '}
                      {price ? (parseFloat(price) * 0.975).toFixed(4) : '0'}{' '}
                      {currency}
                    </p>
                  </div>
                </div>
