...
impl Default for CliConfig {
	fn default() -> Self {
		Self {
			...
			wallet: WalletConfig { default_path: None, auto_unlock: false },
			storage: StorageConfig {
				path: dirs::data_dir().unwrap_or_else(|| PathBuf::from(".")).join("neo-cli"),
			},
...
impl CliConfig {
	...
	fn config_path() -> Result<PathBuf> {
		let config_dir = dirs::config_dir().unwrap_or_else(|| PathBuf::from(".")).join("neo-cli");
