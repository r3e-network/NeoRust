...
impl ConnectionPool {
	async fn get_best_client(
		&self,
	) -> Result<
		&neo3::providers::RpcClient<neo3::providers::HttpProvider>,
...
async fn analyze_recent_transactions(
	...
	if tx_count > 0 {
		println!(
			"      • Average fee: {} GAS",
			(total_fees as f64 / tx_count as f64) / 100_000_000.0
		);
	}
		...
		Ok(result) =>
			if let Some(stack) = result.stack {
				if let Some(item) = stack.first() {
					println!(
						"   ⛽ GAS Symbol: {}",
						item.value.as_ref().and_then(|v| v.as_str()).unwrap_or("GAS")
					);
				}
			},
...
async fn monitor_network_health(
	...
	match client.get_state_height().await {
		Ok(state) => {
			let sync_percentage =
				(state.local_root_index as f64 / state.validated_root_index as f64) * 100.0;
