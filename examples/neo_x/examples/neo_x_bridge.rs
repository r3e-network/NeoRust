...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let bridge_config = BridgeConfig {
		neo_bridge_contract: neo3::neo_types::ScriptHash::from_str("0x48c40d4666f93408be1bef038b6722404d9a4c2a")?,
		neox_bridge_address: "0x85CfE7245BBaED6Df8a501e99656CD503FdF0937", // Example Neo X bridge
		gas_token_neo: neo3::neo_types::ScriptHash::from_str("d2a4cff31913016155e38e474a2c06d08be276cf")?,
...
/// Connect to Neo N3 MainNet
async fn connect_to_neo_mainnet() -> Result<neo3::providers::RpcClient<neo3::providers::HttpProvider>, Box<dyn std::error::Error>> {
	let endpoints = vec![
	...
	match client.get_contract_state(&config.neo_bridge_contract).await {
		Ok(state) => {
			...
			if let Some(manifest) = &state.manifest {
				println!("   📝 Contract name: {}", manifest.name.as_ref().unwrap_or(&"Neo X Bridge".to_string()));
			}
		},
		...
		Ok(result) => {
			if let Some(stack) = result.stack {
				if let Some(item) = stack.first() {
					let is_paused = item.value.as_ref()
						.and_then(|v| v.as_bool())
						.unwrap_or(false);
					println!("   🚦 Bridge status: {}", if is_paused { "PAUSED ⚠️" } else { "ACTIVE ✅" });
				}
			}
		},
...
async fn query_supported_tokens(
	...
	let neo_token = neo3::neo_types::ScriptHash::from_str("ef4073a0f2b305a38ec4050e4d3d28bc40ea63f5")?;
	match client.invoke_function(&config.neo_bridge_contract, "isTokenSupported", &[
		neo3::neo_types::ContractParameter::hash160(&neo_token),
	]).await {
		Ok(result) => {
			if let Some(stack) = result.stack {
				if let Some(item) = stack.first() {
					let supported = item.value.as_ref()
						.and_then(|v| v.as_bool())
						.unwrap_or(false);
					if supported {
						println!("   🪙 NEO Token:");
...
async fn demonstrate_deposit_process(
	...
	script_builder.contract_call(
		...
		"transfer",
		&[
			neo3::neo_types::ContractParameter::hash160(&neo3::neo_types::ScriptHash::from_address(user_address)?),
			neo3::neo_types::ContractParameter::hash160(&config.neo_bridge_contract),
			neo3::neo_types::ContractParameter::integer(deposit_amount),
			neo3::neo_types::ContractParameter::any(neox_recipient),
		],
	...
	println!("      💵 Total cost: ~{} GAS", (config.bridge_fee + 1_000_000) as f64 / 100_000_000.0);
