...
export interface WalletState {
  ...
  importAccount: (walletId: string, privateKey: string, label?: string) => Promise<Account>;
      ...
      (set, get) => ({
        ...
        closeWallet: async (walletId: string) => {
          try {
            ...
            set({ 
              error: error instanceof Error ? error.message : 'Failed to close wallet'
            });
          }
        },
        ...
        deleteWallet: async (walletId: string) => {
          ...
          try {
            ...
            const newCurrentWallet = currentWallet?.id === walletId ? null : currentWallet;
        ...
        importAccount: async (walletId: string, privateKey: string, label?: string) => {
          set({ isLoading: true, error: null });
          try {
        ...
        exportAccount: async (address: string, password: string) => {
          try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Failed to export account';
        ...
        refreshBalances: async (address: string) => {
          try {
            const balances = await invoke<Balance[]>('get_balance', { address });
        ...
        refreshTransactions: async (address: string) => {
          try {
            const transactions = await invoke<Transaction[]>('get_transaction_history', {
              address
            });
